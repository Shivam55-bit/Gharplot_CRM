...
const EmployeeManagementScreen = ({ navigation }) => {
  ...
  const loadEmployees = async (page = 1, search = searchText, statusFilter = filterStatus) => {
    try {
      setLoading(page === 1);
      setError(null);
      console.log('🔥 Loading employees with:', { page, search, statusFilter });
      ...
      console.log('✅ Got employees from API:', fetchedEmployees.length);
      
      // Process employees 
      const processedEmployees = fetchedEmployees.map(emp => ({
        id: emp._id || emp.id,
      ...
      console.log('✅ Processed and set employees:', processedEmployees.length);
      
    } catch (error) {
      console.error('❌ Error loading employees:', error);
      setError(error.message);
      setEmployees([]);
      // You can add a toast/alert here to show the error to user
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      if (stats.fallback) {
        ...
        // Set stats based on current employee data
        const activeCount = employees.filter(emp => emp.status === 'Active').length;
        const inactiveCount = employees.filter(emp => emp.status === 'Inactive').length;
        setDashboardStats({
          totalEmployees: employees.length || 5, // Demo count if no employees loaded yet
          activeEmployees: activeCount || 4, // Demo active count
          inactiveEmployees: inactiveCount || 1, // Demo inactive count
        });
      } else {
      ...
      // Fallback to employee list count if available
      console.log('🔄 Using employee list fallback for stats');
      setDashboardStats({
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            console.log('➕ Plus button pressed - opening Add Employee modal');
